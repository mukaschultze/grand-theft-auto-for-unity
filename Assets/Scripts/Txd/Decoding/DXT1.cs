    ...
    public class DXT1 : Decoder {
        ...
        public override UnityTexture DecodeTextureWithProcessor(BufferReader reader, int width, int height, RasterFormat rasterFormat) {
            using(Timing.Get("DXT1 Decoding")) {
                ...
                var realWidth = width / 4;
                var realHeight = height / 4;
                var bts = reader.ReadBytes(realWidth * realHeight * 8);

                ThreadUtility.For(realWidth, realHeight, (x, y) => {
                    ...
                    var code = BitConverter.ToUInt32(bts, (y * realWidth + x) * 8);
                    var indices = BitConverter.ToUInt32(bts, ((y * realWidth + x) * 8) + 4);
                ...
                using(Timing.Get("DXT1 Decoding (Applying)")) {
